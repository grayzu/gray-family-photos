...
<template>
  ...
  >
    <div class="bg-surface rounded-lg shadow-xl border border-border-subtle max-w-md w-full p-6">
      <h2 class="text-lg font-semibold mb-1 text-text-primary">Photo details</h2>
      <p class="text-sm text-text-muted mb-4">
        {{ fileName ?? "This photo" }} needs a date and location.
        EXIF didn't include both.
      </p>
      ...
      <div class="mt-2">
        <div v-if="loadingLocations" class="text-xs text-text-muted">Searching...</div>
        <div
        ...
        >
          <li v-for="c in candidates" :key="c.placeId">
            <button
              type="button"
              @click="selected = c; query = c.display"
              :class="[
            ...
            >
              ...
              <div
                :class="[
                  'text-[10px]',
                  selected?.placeId === c.placeId ? 'text-base/70' : 'text-text-muted',
                ]"
              >
                {{ c.display }}
              </div>
